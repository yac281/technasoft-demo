...
<section id="page-content" class="col px-0 pb-5 mb-5 pb-lg-4 mb-lg-0">
    <div class="form-group col-auto ml-2">
        <a href="{{route('points.create')}}"
           class="btn btn-sm btn-icon btn-outline-secondary px-3" title="Crea Point">

            <span class="d-none d-md-inline">Crea Point</span>
        </a>
    </div>
    ...
    <div class="px-3">
        <section class="card no-after shadow-y0 bg-white rounded">
            ...
            <table id="data-table" class="table table-striped table-hover table-sm w-100 point_datatable">
                <thead>
                <tr class="text-muted">
...
<script type="text/javascript">
    $(function () {
      var table = $('.point_datatable').DataTable({
        processing: true,
        pageLength: 25,
        deferRender: true,
        "scrollX": true,
        ...
        ajax: "{{ route('points.index') }}",
        columnDefs : [
                { className: "pl-4", targets: [0]},
                { className: "py-0 pl-0 text-right fit", targets: [1]},
            ],
        columns: [
                //{ data: 'id', name: 'id' },
                { data: 'point_name', name:'point_name'},
