...
@section('title', 'Modifica point: '.$point->point_name)
@section('page-title', $point->point_name)
@section('content')
    <!-- Side Bar principale -->
            ...
            <section class="card no-after shadow-y0 bg-white rounded">
                <div class="card-header rounded-top border-0 px-4 py-0 d-flex">
                    <header class="d-flex align-items-center">
                        <h2 class="card-title mb-0 text-primary text-truncate">Modifica dati</h2>
                    </header>
                </div>
                <div class="card-body">
                    {!! Form::open(['route'=>['points.update', $point->id], 'method'=>'POST', 'class'=>'', 'id'=>'edit-item']) !!}
                    <div class="form-row">
                        <div class="form-group form-boxed col-md-6">
                            {!! Form::label('point_name', 'Nome '.'<b class="text-danger">* <span class="sr-only">'.__('forms.required').'</span></b>','', false) !!}
                            {!! Form::text('point_name', old('point_name',$point->point_name), ['class' => 'form-control form-control-sm ']) !!}
                        </div>
                    </div>
